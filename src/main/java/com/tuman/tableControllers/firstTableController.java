...
public class firstTableController extends mainController implements Initializable {
    @FXML
...
cashPay.setOnAction(e-> {
   ...
   money.setCashRevenue( mainController.money.getCashRevenue() + Main.firstTable.getSum());
...
staticStuffRevenueLabel.setText(String.valueOf(money.getTotalMoney() * 0.1));
    Main.firstTable.deleteAllPositions();

stage.close();
});
cardPay.setOnAction(e-> {
  money.setCardRevenue( mainController.money.getCardRevenue() + Main.firstTable.getSum());
    staticRevenueLabel.setText(String.valueOf(money.getTotalMoney()));
...
transferPay.setOnAction(e -> {
    mainController.money.setTransferRevenue( mainController.money.getTransferRevenue() + Main.firstTable.getSum());
...
stage.showAndWait();
    ...
    void addPosition(ActionEvent event) {
        if (posIsNotNull(firstPos) ) {
            Main.firstTable.addPos( firstPos.getValue().toString(), Integer.parseInt(firstCount.getText()), Integer.parseInt(firstPrice.getText()));
        }
        if (posIsNotNull(secondPos)) {
            Main.firstTable.addPos( secondPos.getValue().toString(), Integer.parseInt(secondCount.getText()), Integer.parseInt(secondPrice.getText()) );
        }
        if (posIsNotNull(thirdPos)) {
            Main.firstTable.addPos( thirdPos.getValue().toString(), Integer.parseInt(thirdCount.getText()), Integer.parseInt(thirdPrice.getText()));
        }
        if (posIsNotNull(fourPos)) {
            Main.firstTable.addPos( fourPos.getValue().toString(), Integer.parseInt(fourCount.getText()), Integer.parseInt(fourPrice.getText()));
        }
       clearAllPos();
    }
    ...
    void openTableCheck(ActionEvent event) {
        ...
        if (!Main.firstTable.getPositions().isEmpty()){
        for (int i = 0; i< (Main.firstTable.getPositions().size()); i++){
            lis.add(new tableModel(Main.firstTable.getPositions().get(i),String.valueOf(Main.firstTable.getAmount().get(i)),String.valueOf(Main.firstTable.getPrice().get(i)),
                    String.valueOf(Main.firstTable.getAmount().get(i)*Main.firstTable.getPrice().get(i)) ));
        }}
        ...
        Label totalLabel = new Label("Итого:" + Main.firstTable.getSum());
        ...
        changeButton.setOnAction(e->
        {
            ...
            if (!table.getSelectionModel().isEmpty()) {
                ...
                confirm.setOnAction(o-> {
                    Main.firstTable.changePositionValues(table.getSelectionModel().getFocusedIndex(), fieldPosition.getText(), fieldAmount.getText(), fieldPrice.getText());
                    st.close();
                    stage.close();
                });
                deletePosition.setOnAction(p -> {
                    Main.firstTable.deletePositionValues(table.getSelectionModel().getFocusedIndex());
                    st.close();
                    stage.close();
                });
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        System.out.println(mainController.money.getCashRevenue());
