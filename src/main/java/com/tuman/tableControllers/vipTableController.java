...
public class vipTableController extends mainController implements Initializable {
    @FXML
    ...
    void CountUp(ActionEvent event) {
        ...
        cashPay.setOnAction(e-> {
            ...
            money.setCashRevenue( money.getCashRevenue() + Main.vipTable.getSum());
            ...
            Main.vipTable.deleteAllPositions();

            stage.close();
        });
        cardPay.setOnAction(e-> {
            money.setCardRevenue( money.getCardRevenue() + Main.vipTable.getSum());
            staticRevenueLabel.setText(String.valueOf(money.getTotalMoney()));
        ...
        transferPay.setOnAction(e -> {
            money.setTransferRevenue( money.getTransferRevenue() + Main.vipTable.getSum());
    ...
    void addPosition(ActionEvent event) {
        if (posIsNotNull(firstPos) ) {
            Main.vipTable.addPos( firstPos.getValue().toString(), Integer.parseInt(firstCount.getText()), Integer.parseInt(firstPrice.getText()));
        }
        if (posIsNotNull(secondPos)) {
            Main.vipTable.addPos( secondPos.getValue().toString(), Integer.parseInt(secondCount.getText()), Integer.parseInt(secondPrice.getText()) );
        }
        if (posIsNotNull(thirdPos)) {
            Main.vipTable.addPos( thirdPos.getValue().toString(), Integer.parseInt(thirdCount.getText()), Integer.parseInt(thirdPrice.getText()));
        }
        if (posIsNotNull(fourPos)) {
            Main.vipTable.addPos( fourPos.getValue().toString(), Integer.parseInt(fourCount.getText()), Integer.parseInt(fourPrice.getText()));
        }
        clearAllPos();
    }
    ...
    void openTableCheck(ActionEvent event) {
        ...
        if (!Main.vipTable.getPositions().isEmpty()){
            for (int i = 0; i< (Main.vipTable.getPositions().size()); i++){
                lis.add(new tableModel(Main.vipTable.getPositions().get(i),String.valueOf(Main.vipTable.getAmount().get(i)),String.valueOf(Main.vipTable.getPrice().get(i)),
                        String.valueOf(Main.vipTable.getAmount().get(i)*Main.vipTable.getPrice().get(i)) ));
            }}
        ...
        Label totalLabel = new Label("Итого:" + Main.vipTable.getSum());
        ...
        changeButton.setOnAction(e->
        {
            ...
            if (!table.getSelectionModel().isEmpty()) {
                ...
                confirm.setOnAction(o-> {
                    Main.vipTable.changePositionValues(table.getSelectionModel().getFocusedIndex(), fieldPosition.getText(), fieldAmount.getText(), fieldPrice.getText());
                    st.close();
                    stage.close();
                });
                deletePosition.setOnAction(p -> {
                    Main.vipTable.deletePositionValues(table.getSelectionModel().getFocusedIndex());
                    st.close();
                    stage.close();
                });
